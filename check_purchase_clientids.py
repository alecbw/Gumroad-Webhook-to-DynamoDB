...
def lookup_GA_clientid(client_id):
    ...
    ga_cid_lookup_url += "&dimensions=" + "ga:dataSource,ga:dateHourMinute,ga:fullReferrer,ga:sourceMedium"
    ...
    ga_output_dict = {
        ...
        "sessions": sum([int(x[4]) for x in resp_lol]),
        "totalEvents": sum([int(x[5]) for x in resp_lol]),
        "eventValue": sum([int(x[6]) for x in resp_lol]),
...
if __name__ == "__main__":
    ...
    header = ["sale_readable", "value", "country", "first_event", "first_sourcemedium", "dataSource", "dateHourMinute", "referralPath", "fullReferrer", "sourceMedium", "sessions", "totalEvents", "eventValue", "email", "cid", "offer_code",  "data", "updatedAt", "timestamp", "_ga", "refunded", "ml_created", "ml_source", "signup_keyword", "signup_search_query", "signup_sourcemedium", "signup_referrer", "signup_ga_timestamp", "signup_clientid"]
